...
object MongoMigration {
  def run[O](mongoClient: Stream[IO, MongoClient], callback: User => Stream[IO, O])(implicit C: ConcurrentEffect[IO]): Stream[IO, O] =
    for {
      ...
      outcome <- user.fold(Stream.raiseError[IO], callback)
    } yield {
